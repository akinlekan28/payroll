...
router.get('/download', (req, res) => {
    let template = path.join(__basedir, '../../', 'docs/Employee_Record_Template.xlsx')
...
const importToDb = filePath => {
    ...
    excelData.Employees.forEach(employee => {
        ...
        .then(level => {
            if(level != null){
                Employee.findOne({email}).where('is_delete').equals(0)
                .then(employeeRecord => {
                    if(!employeeRecord){
                        const newEmployee = new Employee({
                          ...
                          newEmployee.save().then(employeeDetails =>{
